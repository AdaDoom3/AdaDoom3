...
void main() {
	vec4 accumSample = tex2D ( samp0 , vofi_TexCoord0 ) ;
	vec4 currentRenderSample = tex2D ( samp1 , vofi_TexCoord0 ) ;
	vec4 maskSample = tex2D ( samp2 , vofi_TexCoord0 ) ;
	gl_FragColor = mix ( accumSample , currentRenderSample , maskSample. a ) ;
}