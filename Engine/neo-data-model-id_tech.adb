...
separate (Neo.Data.Model) package body Id_Tech is
  ...
  function Load (Path : Str) return Camera_State is
    ...
    begin
      ...
      while Peek /= "}" loop Assert ("(");
        Camera.Frames.Append ((Point       => (Next, Next, Next_Then_Assert (")", "(")),
    ...
    begin
      ...
      while not At_EOF loop Assert ("mesh", "{", "shader");
        ...
        while Peek = "vert" loop Skip (2); Assert ("(");
          Surface.Vertices.Append ((Texture      => (Next, Next_Then_Assert (")")),
                                    Weight_Count => Next,
                                    Start_Weight => Next, others => <>));
        end loop; 
  ...
  function Load (Path : Str) return Animation_State is
    ...
    begin
      ...
      Assert ("hierarchy", "{");
      while Peek /= "}" loop
        Frame_Base.Append ((Data => (Name         => To_Joint_Name (Next_Set ("""", """")),
                                     Parent_Index => Next,
                                     others       => <>),
                            Flags       => Next,
                            Start_Index => Next));
      ...
      for Joint of Frame_Base loop
        ...
        Joint.Data.Point       := (Next, Next, Next); Assert (")", "(");
        Joint.Data.Orientation := To_Quaternion_4D ((Next, Next, Next)); Assert (")");
      end loop; Skip;
      ...
      while not At_EOF loop
        ...
        for Base of Frame_Base loop
          Joint := Base.Data;
          if (Base.Flags and 16#01#) > 0 then Joint.Point.X       := Frame_Data.Element (I); I := I + 1; end if;
          if (Base.Flags and 16#02#) > 0 then Joint.Point.Y       := Frame_Data.Element (I); I := I + 1; end if;
          if (Base.Flags and 16#04#) > 0 then Joint.Point.Z       := Frame_Data.Element (I); I := I + 1; end if;
          if (Base.Flags and 16#08#) > 0 then Joint.Orientation.X := Frame_Data.Element (I); I := I + 1; end if;
          ...
          while Peek /= "}" loop
            ...
            -- Handle geometry
              ...
              if Token = "brushDef3" then
                ...
                while Peek /= "}" loop
                  Assert ("(");
                  Brush.Append ((Plane   => (Next, Next, Next, Next_Then_Assert (")", "(", "(")),
                                 Texture => (XX => Next, YX => Next, ZX => Next_Then_Assert (")", "("),
                                             XY => Next, YY => Next, ZY => Next_Then_Assert (")", ")")),
                                 Material => Next_Set ("""", """"),
              ...
              -- Load patch
                ...
                while Peek /= ")" loop Assert ("(");
                  while Peek /= ")" loop Assert ("(");
                    Patch.Vertices.Append (((Next, Next), (Next, Next, Next) - Entity.Point, others => <>)); Assert (")"); 
                  end loop; Assert (")");
                end loop; Skip;
