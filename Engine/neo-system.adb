package body Neo.System is
  ...
  procedure Assert        (Value : in Address)              is begin Assert(Value /= NULL_ADDRESS);                                                                                                                                             end Assert;
  procedure Assert        (Value : in Boolean)              is begin if not Value then Trace; raise Call_Failure; end if;                                                                                                                       end Assert;
  ...
  procedure Trace is
    ...
    begin
      ...
      begin
        while Traceback(Current) /= Character_2'Val(Character_1'Pos(ASCII.LF)) and Current /= Traceback'Last loop
