...
package Neo.Model is
  ...
  type Frame_State is record
      Point         : Point_3D;
      Orientation   : Vector_Quaternion_4D.Unsafe.Vector;
      Field_Of_View : Real;
    end record;
  ...
  package Vector_Quaternion_4D is new Vectors (Quaternion_4D);
  ...
  package Ordered_Vector_Vertex        is new Ordered_Maps (Int_32_Natural, Vector_Vertex.Unsafe.Vector);
  package Vector_Ordered_Vector_Vertex is new Vectors      (Ordered_Vector_Vertex.Unsafe.Map);

  -- Vertex group
  type Group_State is record
      Material : Str_Unbound;
      Shapes   : Vector_Ordered_Vector_Vertex.Unsafe.Vector;      
    end record;
