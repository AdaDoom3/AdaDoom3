package body Neo.System.Window
  ...
  ------------
    ...
    package Instantiation is new Import(Handle_Finalization, Handle_Activation, Handle_State_Change, Handle_Window_Move, Handle_Resize);
  -------------------------------
  -- Task_Multi_Monitor_Window --
  -------------------------------
    task body Task_Multi_Monitor_Window is
      Index   : Integer_4_Positive := 1;
      Do_Quit : Boolean := False;
      begin
        accept Initialize(I : in Integer_4_Positive) do 
            Index := I;
          end Initialize;
  ...
  -- Handle_Window_Move --
  ------------------------
    procedure Handle_Window_Move(Window_X, Window_Y, Screen_X, Screen_Y : in Integer_4_Signed) is
      begin
        Window.X := Window_X;
        Window.Y := Window_Y;
        Protected_Data.Set(Window);
        if Protected_Data.Get.State = Windowed_State then
          Center := (Screen_X + Protected_Data.Get.Width  / 2, Screen_Y + Protected_Data.Get.Height / 2);
        end if;
      end Handle_Window_Move;
  -------------------------
  -- Handle_State_Change --
  -------------------------
    procedure Handle_State_Change(Change : in Enumerated_Window_Change) is
      begin
        case Change is
          when Iconic_Change =>
  ...
  -- Handle_Activation --
  -----------------------
    procedure Handle_Activation(Do_Activate, Do_Detect_Click : in Boolean; X, Y : in Integer_4_Signed) is
      begin
  ...
  -- Handle_Reshape --
  --------------------
    function Handle_Reshape(Resize_Location : in Enumerated_Resize; Current_Screen : in Record_Window_Border) return Record_Window_Border is
