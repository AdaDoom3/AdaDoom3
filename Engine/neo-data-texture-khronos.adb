...
separate (Neo.Data.Texture) package body Khronos is
  ...
  function Load (Path : Str) return Compressed_Image is
    ...
    begin
      Target := (if Header.NumberOfFaces > 1 then
                   (if Header.NumberOfArrayElements > 0 then TARGET_CUBE_ARRAY else TARGET_CUBE)
                 elsif Header.NumberOfArrayElements > 0 then
                   (if Header.PixelHeight = 0 then TARGET_1D_ARRAY else TARGET_2D_ARRAY)
                 elsif Header.PixelHeight = 0 then TARGET_1D
                 elsif Header.PixelDepth > 0 then TARGET_3D
                 else TARGET_2D);
