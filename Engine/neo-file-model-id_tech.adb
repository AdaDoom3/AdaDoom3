package Id_Tech is
  ...
  function return Record_Animation is
    ...
    begin
      ...
      Parser.Assert("bounds"); Parser.Assert("{");
      while Parser.Peek /= "}" loop
        
      end loop; Parser.Skip;
      Parser.Assert("baseframe"); Parser.Assert("{");
      while Parser.Peek /= "}" loop
  ...
  function return Record_Mesh is
    ...
    begin
      ...
      Parser.Assert("commandline"); Parser.Skip_Set("""", """");
      Parser.Assert("numJoints");   Parser.Skip;
      Parser.Assert("numMeshes");   Parser.Skip;
      Parser.Assert("joints");      Parser.Assert("{");
      while Parser.Peek /= "}" loop
        Joints.Append(
          Name     => Parser.Next_Set("""", """"),
          Parent   => Parser.Next,
      ...
      while not Parser.At_End loop
        Parser.Assert("mesh");     Parser.Assert("{");
        Parser.Assert("shader");   Surface.Shader := Parser.Next;
        Parser.Assert("numverts"); Parser.Skip;
