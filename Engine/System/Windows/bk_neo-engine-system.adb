...
separate (Neo.Engine) package body System is
  ...
  -- Window titles
  GAME_NAME          : aliased  Str_C := To_Str_C (Game_Info.NAME_ID);
  INPUT_NAME         : aliased  Str_C := To_Str_C (Game_Info.NAME_ID & " Input");
  CONSOLE_NAME       : aliased  Str_C := To_Str_C (Game_Info.NAME_ID & " Console");
  MULTI_MONITOR_NAME : aliased  Str_C := To_Str_C (Game_Info.NAME_ID & " Multi-monitor");
  ...
  function Ok (Message : Str; Buttons : Buttons_Kind; Icon : Icon_Kind) return Bool is
    ...
    function CBTProc (nCode : Int_C; wParam, lParam : Int_Ptr) return Int_Ptr is
      ...
      begin
        ...
        if nCode = HCBT_ACTIVATE and S (Class_Name) = S (DIALOG_CLASS) and S (Window_Text) = Game_Info.NAME_ID then
