...
begin
   ...
   if Do_Put_Debug then 
      Neo.System.Text.Console.Initialize; 
   end if;
   
   begin Run; 
   exception 
      when Unimplemented_Feature => raise;
      when Occurrence: others => Handle_Exception(Occurrence); 
   end;
   
   if not Neo.System.Text.Console.Is_Running and then Do_Fail and then 
     Is_Okay
     (Icon    => Error_Icon,
      Name    => To_String_2(Neo.System.SPECIFICS.Name),
      Message => Localize("An error has occurred, would you like to view more information?"),
      Buttons => Yes_No_Buttons) then
