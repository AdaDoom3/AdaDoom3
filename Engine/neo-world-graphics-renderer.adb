...
separate (Neo.World.Graphics) package body Renderer is

  -- Perspective matricies
  package Model_Matrix      is new Uniform (Binding => 22, Uniform_T => Matrix_4D, Name_ID => "model");
  package View_Matrix       is new Uniform (Binding => 23, Uniform_T => Matrix_4D, Name_ID => "view");
  package Projection_Matrix is new Uniform (Binding => 24, Uniform_T => Matrix_4D, Name_ID => "projection");

  --package All_The_Stuff is new Uniform (Uniform_State, );
  package Test_Shader is new Shader ("test", ((Kind     => Fragment_Stage,
                                               --Uniforms => (),
                                               others   => <>),
                                              (Kind     => Vertex_Stage,
                                               Uniforms => (U ("model"), U ("view"), U ("projection"), others => NULL_STR_UNBOUND),
                                               others   => <>)));
  ...
  procedure Build_Frame (Frame : in out Framebuffer_State; View : View_State) is
    ...
    -- Local MVP
    Frame_Projection : constant Matrix_4D := Perspective (CAMERA_FOV, Real (Window_Width.Get) / Real (Window_Height.Get), 0.1, 2000.0);
    Frame_View       : constant Matrix_4D := Camera.View;

    -- Start of Build_Frame
    begin
      for Surface of View.Surfaces loop

        -- State
        State.Mesh := Surface.Mesh;
        ...
        -- MVP
        Projection_Matrix.Set (Frame_Projection);
        View_Matrix.Set       (Frame_View);
        Model_Matrix.Set      (IDENTITY_MATRIX_4D);

        -- Actually draw
        Draw (State, Frame, (others => True));
      end loop;
      --
    end;
  ...
  procedure Build_View (View : in out View_State) is
    Bottom_Level : Bottom_Level_State;
    begin
      if In_Game.Occupied then
        for Mesh of Map.Get.Geometry loop
          if Mesh /= null then
            View.Surfaces.Append ((Mesh => Buffered_Meshes.Get (Geometry_Name_Str.S (Mesh.Name)), others => <>));
          end if;
        end loop;
      end if;
      --for Entity of Entities.Get loop
      --  View.Surfaces.Append ((Mesh => Buffered_Meshes.Get (Entity.Mesh_Name), others => <>));
      --end loop;
      --
    end;
  end;
