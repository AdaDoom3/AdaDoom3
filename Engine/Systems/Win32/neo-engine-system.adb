...
separate (Neo.Engine) package body System is
  ...
  function Create_Vulkan_Surface (Instance : Ptr) return Ptr is
    ...
    Surface_Info : aliased VkWin32SurfaceCreateInfoKHR := (hWnd => Game_Window, hInstance => GetModuleHandleW (null), others => <>);
    begin
      Assert (Game_Window); -- The window must be created already
  ...
  function Ok (Message : Str; Buttons : Buttons_Kind; Icon : Icon_Kind) return Bool is
    ...
    begin
      ...
      -- Do the call
      return (case MessageBoxW (hWnd      => Game_Window,
                                lpCaption => C (GAME_NAME),
                                lpText    => C (C_Message),
                                uType     => (if Game_Window = NULL_PTR then MB_SYSTEMMODAL else 0)
                                                or (case Icon is
                                                      when No_Icon          => 0,
  ...
  procedure Minimize is begin Ignore (ShowWindow (Game_Window, SW_SHOWMINIMIZED) = 0); end; 
  ...
  procedure Adjust_Windowing (X, Y : Int; Width, Height : Positive; Fullscreen : Bool) is
    begin
      if Game_Window = NULL_PTR then
        Game_Window := CreateWindowExW (dwExStyle    => 0,
                                        ...
                                        lpParam      => NULL_PTR);
        Assert (Game_Window);
        Ignore (ShowWindow (Game_Window, SW_RESTORE));
      else
        Assert (SetWindowLongW (Game_Window, GWL_STYLE, (if Fullscreen then STYLE_FULLSCREEN else STYLE_WINDOWED)) /= 0);
        Assert (SetWindowPos (hWnd            => Game_Window,
  ...
  procedure Set_Cursor_Style_Internal (Kind : Ptr) is
    begin
      if WORD_SIZE = 32 then Ignore (SetClassLongW (Game_Window, GCLP_HCURSOR, Int_Unsigned_C (To_Int_Ptr (Kind))));
      else Ignore (SetClassLongPtrW (Game_Window, GCLP_HCURSOR, To_Int_Ptr (Kind))); end if;
    end;
  ...
  procedure Clip_Cursor (Do_Clip : Boolean := True) is
    ...
    begin
      Assert (GetWindowRect (Game_Window, Rectangle'Unchecked_Access));
  ...
  function Game_Window_Proc (hwnd : Ptr; uMsg : Int_Unsigned_C; wParam, lParam : Int_Ptr) return Int_Ptr is
    begin    
      case uMsg is
        when WM_DESTROY => PostQuitMessage (0); Close_Game := True; return 0;
  ...
  procedure Initialize_Windowing is
    ...
    begin
      ...
      Class := (lpfnWndProc   => Game_Window_Proc'Address,
                ...
                hbrBackground => To_Int_Ptr (CreateSolidBrush (To_Windows_Color (COLOR_BLACK))),
  ...
  function Update_Windowing return Bool is
    ...
    begin
      while PeekMessageW (Message'Access, Game_Window, 0, 0, PM_REMOVE) /= 0 loop
  ...
  procedure Finalize_Windowing is
    begin
      if Game_Window /= NULL_PTR then
        Ignore (ShowWindow (Game_Window, SW_HIDE) = 0);
        Ignore (DestroyWindow (Game_Window));
      end if;
      Ignore (UnregisterClassW (C (Game_Name), NULL_PTR));
      Game_Window := NULL_PTR;
    end;
  ...
  function Get_Windows return Vector_Border.Unsafe_Array is
    begin
      ...
      begin
        Assert (GetWindowRect (Game_Window, Result'Unchecked_Access));
  ...
  function Get_Cursor return Cursor_state is
    ...
    begin
      Ignore (GetCursorPos (Pt'Unchecked_Access));
  ...
  function Input_Window_Proc (hwnd : Ptr; uMsg : Int_Unsigned_C; wParam, lParam : Int_Ptr) return Int_Ptr is
    ...
    begin
      case uMsg is
        when WM_DESTROY      => PostQuitMessage (0); Close_Input := True; return 0;
  ...
  procedure Initialize_Input is
    ...
    begin
      ...
      Assert (RegisterClassExW (Class'Unchecked_Access));
      Input_Window := CreateWindowExW (dwExStyle    => 0,
      ...
      declare
      Setups : aliased array (1..2) of RAWINPUTDEVICE := ((GENERIC_DESKTOP, USE_RAW_KEYBOARD, RIDEV_INPUTSINK, Input_Window),
                                                          (GENERIC_DESKTOP, USE_RAW_MOUSE,    RIDEV_INPUTSINK, Input_Window));
      begin
  ...
  procedure Finalize_Input is
    ...
    begin
      ...
      if Input_Window /= NULL_PTR then Ignore (DestroyWindow (Input_Window)); end if;
      Input_Window := NULL_PTR;
    end;
  ...
  function Update_Input return Bool is
    ...
    begin
      ...
      while PeekMessageW (Message'Unchecked_Access, Input_Window, 0, 0, PM_REMOVE) /= 0 loop
        Ignore (TranslateMessage (Message'Unchecked_Access));
        Ignore (DispatchMessageW (Message'Unchecked_Access));
