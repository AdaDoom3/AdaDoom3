...
separate (Neo.Engine.System) package body Console is
      ...
      function EnumWindowsProc (hwnd : Ptr; lParam : Int_Ptr) return Int_C is
        ...
        begin
          ...
          if DBU_Width = 0 and "Button" = S (Class_Name) then
            ...
            Ignore (SelectObject    (Context, To_Ptr (SendMessageW (hwnd, WM_GETFONT, 0, 0)))); -- This fails without -mwindows
            Assert (GetTextMetricsW (Context, Text_Metric'Unchecked_Access));
      ...
      case uMsg is
        ...
        when WM_CTLCOLORSTATIC | WM_CTLCOLOREDIT =>
          if To_Ptr (lParam) = Output_Box or To_Ptr (lParam) = Input_Box then
            Assert (SetBkColor   (To_Ptr (Int_Ptr (wParam)), To_Windows_Color (CONSOLE_BACKGROUND_COLOR)) /= CLR_INVALID);
            Assert (SetTextColor (To_Ptr (Int_Ptr (wParam)), To_Windows_Color (CONSOLE_FOREGROUND_COLOR)) /= CLR_INVALID);
        ...
        when WM_SIZE =>
          if wParam /= Int_Ptr (SIZE_MINIMIZED) then
            ...
            Ignore (SetWindowLongW (Console_Window, GWL_EXSTYLE, WS_EX_COMPOSITED));
            Assert (GetWindowRect  (Console_Window, Rectangle'Unchecked_Access));
            ...
            -- Apply new sizes
            Assert (SetWindowPos (hWnd            => Console_Window,
            ...
            Ignore (SetWindowLongW (Console_Window, GWL_EXSTYLE, 0));
            return 1;
          end if;
  ...
  procedure Run is 
    begin 
      ...
      Console_Window := CreateWindowExW (lpClassName  => C (CONSOLE_NAME),
