...
separate(Neo.System) package body Import is
    ...
    procedure Set_Alert(Value : in Boolean) is
      ...
      Flash_Information : aliased Record_Flash_Information :=
        (if Value then (Flags => FLASH_CONTINUOUSLY, others => <>) else (Flags => FLASH_END, others => <>));
      begin
        Assert(Window);
    ...
    function Is_Okay(Name, Message : in String_2; Buttons : in Enumerated_Buttons; Icon : in Enumerated_Icon) return Boolean is
      ...
      function Hook(Code : in Integer_4_Signed_C; Data_Unsigned : in Integer_4_Unsigned_C; Data_Signed : in Integer_4_Signed_C) return Integer_4_Signed_C;
        pragma Convention(Stdcall, Hook);
      function Hook(Code : in Integer_4_Signed_C; Data_Unsigned : in Integer_4_Unsigned_C; Data_Signed : in Integer_4_Signed_C) return Integer_4_Signed_C is
        ...
        return(
          case
            Message_Box(
              ...
              Kind    =>(
                if Window = NULL_ADDRESS then MESSAGE_BOX_SYSTEM_MODAL else 0) -- Force the message box into the foreground
                or(
                  case Icon is
        ...
        Buffer_Directory(Integer_Size_C(Index(To_String_2(Buffer_Directory), "\", Backward))) := NULL_CHARACTER_2_C;
          ...
          begin
            ...
            return(
              ...
              Version  =>(
                case Version.Platform_Identifier is
                  when 1 =>(
                    case Version.Major is
                      when 4 =>(
                        case Version.Minor is
                          when 0 =>(
                            case Version.Service_Pack(2) is
