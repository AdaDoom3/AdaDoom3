...
separate(Neo.System.Text.Console) package body Import is -- "Abandon all hope, you who enter here"
    procedure Run
      ...
      FONT_GROUP_BOX_SIZE     : constant Float                    := 1.5;
      FONT_CONSOLE_SIZE       : constant Float                    := 1.4; -- The amount to divide 1 pt size by
      ...
      Is_Pressing_Control     :         Boolean                   := False;
      ...
      Is_Holding_Control      :         Boolean                   := False;
          ...
          Assert(Send_Message(
            ...
            Data_Signed   =>
                  To_Unchecked_Integer_Address(
                    To_Access_Constant_Character_2_C(Localize(Text)))));  
        end Set_Text;
      procedure Set_Font(Handle : in Address; Font : in Address) is
        begin
          Assert_Dummy(Send_Message(
            ...
            Data_Signed   => 0,
            Data_Unsigned =>
                  To_Unchecked_Integer_Address(Font)));
        end Set_Font;
      procedure Set_Sizes is
        ...
        begin
          ...
          Margin_Group      := Text_Metric.Height;
          ...
          Console_Height    := (MARGIN * Dialog_Base_Unit_Height) * 4 + (Border_Height + Margin_Group + Margin_Group_Top) * 2 + Output_Box_Height + Input_Box_Height + BUTTON_HEIGHT + Title_Bar_Height;
      ...
      function Callback_Console(Window : in Address; Message : in Integer_4_Unsigned_C; Data_Unsigned, Data_Signed : in Integer_Address) return Integer_Address; pragma Convention(Stdcall, Callback_Console);
      function Callback_Console(Window : in Address; Message : in Integer_4_Unsigned_C; Data_Unsigned, Data_Signed : in Integer_Address) return Integer_Address is
        Point : Record_Point;
        ...
        procedure Resize is
          ...
          begin
            ...
            Assert(Set_Window_Position(
              Window       => Output_Box,
              Insert_After => NULL_ADDRESS,
              X            => Margin_Group + Dialog_Base_Unit_Width * MARGIN,--MARGIN_GROUP_SIDE,
              Y            => Y,
            ...
            Assert(Set_Window_Position(
              Window       => Input_Box,
              Insert_After => NULL_ADDRESS,
              X            => Margin_Group + Dialog_Base_Unit_Width * MARGIN,-- + Margin_Group,--MARGIN_GROUP_SIDE),
              Y            => Y,
              Width        => Output_Box_Width,
            ...
            if Send_Message(Output_Box, 206, 0, 0) + Number_Of_Lines + 2 >= Send_Message(Output_Box, 16#00BA#, 0, 0) then
              Assert_Dummy(Send_Message(
                Window        => Output_Box,
      ...
      begin        
        ...
        while Message.Data /= MESSAGE_QUIT loop
          if Get_Number_Of_Lines /= Current_Lines then
            Current_Line := Send_Message(Output_Box, 206, 0, 0);
            Assert(Send_Message(
              Window        => Output_Box,
            ...
            if Integer_8_Natural(Current_Line + Number_Of_Lines) +  Get_Number_Of_Lines - Current_Lines
            >= Integer_8_Natural(Send_Message(Output_Box, 16#00BA#, 0, 0)) or Is_First_Time then
              Is_First_Time := False;
              Assert_Dummy(Send_Message(
                Window        => Output_Box,
                Message       => EVENT_SCROLL_VERTICALLY,
              ...
              for I in Send_Message(Output_Box, 206, 0, 0)..Current_Line - 1 loop
                Assert_Dummy(Send_Message(
                  Window        => Output_Box,
                  Message       => EVENT_SCROLL_VERTICALLY,
                  Data_Unsigned => 1, -- SB_BOTTOM
                  Data_Signed   => 0));
              end loop;
            end if;
            Assert(Send_Message(
            ...
            case Message.Data is
              when EVENT_KEY_DOWN =>
                ...
                case Integer_2_Unsigned_C(Message.Data_Unsigned) is
                  when VIRTUAL_KEY_CONTROL => Is_Pressing_Control := True; 
                  when VIRTUAL_KEY_V =>
                    if Is_Pressing_Control and Get_Focus /= Input_Box then
              ...
              when EVENT_KEY_UP =>
                if Integer_2_Unsigned_C(Message.Data_Unsigned) = VIRTUAL_KEY_CONTROL then Is_Pressing_Control := False; end if;
                  ...
                  if not Is_Control(Current_Input) then
                    Assert_Dummy(
                      Send_Message(
                        Input_Box,
                        16#D#,
                        Integer_Address(Buffer'length),
                        To_Unchecked_Integer_Address(Buffer'address)));
      ...
      exception when O: others => Put_Line(To_String_2(Exception_Name(O)));
      end Run;
  end Import; -- Is it a sign?
