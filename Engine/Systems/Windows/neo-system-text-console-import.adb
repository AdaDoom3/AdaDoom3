...
separate(Neo.System.Text.Console) package body Import is -- "Abandon all hope, you who enter here"
    procedure Run
      ...
      procedure Set_Sizes is
        ...
        Current_Output_Box_Width  : Integer_4_Signed_C := 0;
        Current_Output_Box_Height : Integer_4_Signed_C := 0;
        begin
          ...
          Current_Output_Box_Width  := -No_Output_Width + Border_Width * 2 + Current_Width < Output_Box_Width;
          Current_Output_Box_Height := -No_Output_Height + Border_Height * 2 + Title_Bar_Height + Current_Height;
          Output_Box_Width :=(
            if  then Output_Box_Width
            else
          Output_Box_Height := 
        end Set_Sizes;
      procedure Create_Fonts is 
        begin
          if Did_Fail_Button_Font_Set then
            Font_Buttons :=
              Create_Font(
...
--                  end if;
            ...
            when EVENT_BUTTON_COMMAND =>
              for I in CONSOLE_BUTTONS'range loop
                if Data_Unsigned = Integer_4_Unsigned_C(I + IDENTIFIER_START) then
                  if CONSOLE_BUTTONS(I).Action = null then Send_Message(Console, EVENT_CLOSE, 0, 0); 
      ...
      begin
        ...
        if System_Parameter_Information( -- Fails under XP with no error information....
          ...
          Parameter_A  =>(
            ...
            else Non_Client_Metrics.Size - Integer_4_Unsigned_C(Integer_4_Signed_C'size / Byte'size))) = FAILED then
          Did_Fail_Button_Font_Set := True;
        else
                ...
                if Do_Process_Character then
                  ...
                  if not Is_Control(Current_Input) then
                    ...
                    if Get_Input_Entry /= NULL_STRING_2 then
                      ...
                      Set_Text(Input_Box(Input_Box, NULL_STRING_2));
                    end if;
