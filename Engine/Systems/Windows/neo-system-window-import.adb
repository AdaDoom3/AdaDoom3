...
-- http://social.msdn.microsoft.com/Forums/en-US/windowscompatibility/thread/7b5ef777-ff0d-4f15-afed-5588f93f0e23/
  ...
  ------------------
    function Get_Monitors
      ...
      --------------------------
        ...
        begin
          if Get_Monitor_Information(Monitor, Monitor_Information'access) = FAILED then
            raise Call_Failure;
          end if;
          Monitors(Count) :=(
            Desktop =>(
    ...
    procedure Adjust(
      ...
      begin
        ...
        if Do_Fullscreen then
          if Change_Window_Setting(Primary_Window, SET_WINDOW_STYLE, FULLSCREEN_STYLE) = 0 then
            raise Call_Failure;
          end if;
          if
          Set_Window_Position(
          ...
          Change_Window_Setting(
            Window  => Primary_Window,
            Command => SET_WINDOW_STYLE,
            Setting => WINDOWED_STYLE or STYLE_BOX_FULLSCREEN) = FAILED
          then
            raise Call_Failure;
          end if;
          if
  ...
  -------------------
    function Get_Specifics
      ...
      begin
      -----
        if Create_Class(Callback'address, " ") = FAILED then
          raise Call_Failure;
        end if;
        ...
        Stencil_Bits :=(
          if Pixel_Descriptor.Stencil_Bits /= 0 then -- Windows XP seems to set this incorrectly
            Pixel_Descriptor.Stencil_Bits
          else
            Byte'size);
        if Set_Pixel_Format(Device_Context, Pixel_Format, &win32.pfd) = FAILED then
          raise Call_Failure;
        end if;
        if Create_Context(Device_Context) = FAILED then
          raise Call_Failure;
        end if;
      end Get_Specifics;
  end Import;
