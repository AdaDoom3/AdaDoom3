...
separate(Neo.System.Graphics.Window) package body Import is
  ...
  Primary_Window        :         Address               := NULL_ADDRESS;
  ...
  procedure Set_Custom_Mouse_Internal(Window : in Address; Cursor : in Enumerated_Cursor := Inactive_Cursor) is
    begin
      Cursor_Current := Cursor;
      if WORD_SIZE = 32 then case Cursor is -- Because macros
    ...
    end Set_Custom_Mouse_Internal;
  procedure Set_Custom_Mouse(Cursor : in Enumerated_Cursor := Inactive_Cursor) is begin Set_Custom_Mouse_Internal(Primary_Window, Cursor); end Set_Custom_Mouse;
  procedure Assert_Only_Instance is
    ...
    begin
      ...
      if Get_Last_Error /= NO_ERROR then
        ...
        Primary_Window := Find_Window(To_String_2_C(To_String_2(Neo.System.SPECIFICS.Name)), NULL_ADDRESS);
  ...
  function Update(Index : in Integer_4_Natural := 0) return Boolean is
    ...
    begin
      while Peek_Message(
        ...
        Command        => REMOVE_MESSAGES_AFTER_PROCESSING) /= FAILED loop
