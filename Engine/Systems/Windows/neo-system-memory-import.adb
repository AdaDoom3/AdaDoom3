...
separate(Neo.System.Memory) package body Import is
    ...
    procedure Set_Byte_Limits(Minimum, Maximum : in Integer_8_Unsigned) is
      begin
        Assert(Set_Process_Working_Set_Size(Get_Current_Process, Integer_Size_C(Minimum), Integer_Size_C(Maximum)));
      end Set_Byte_Limits;
    procedure Lock(Location : in Address; Number_Of_Bytes : in Integer_8_Unsigned) is
      begin
        Assert(Virtual_Lock(Location, Integer_Size_C(Number_Of_Bytes)));
      end Lock;
    procedure Unlock(Location : in Address; Number_Of_Bytes : in Integer_8_Unsigned) is
      begin
        Assert(Virtual_Unlock(Location, Integer_Size_C(Number_Of_Bytes)));
      end Unlock;
  end Import;