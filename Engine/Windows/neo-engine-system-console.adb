...
separate (Neo.Engine.System) package body Console is
  ...
  begin 
    ...
    -- Load the icon
    Icon := LoadImageW (hinst     => GetModuleHandleNULL, -- Loads the icon nicely for the Aero theme, but not on the "classic" theme
    ...
    -- Determine the spawn location so we are not hiding behind or blocking a windowed game
    --if Mode.Get = Windowed_Mode and Get_Windows (1).Right - Get_Windows (1).Left /= 0 then
    --  Console_X := Int_C (Get_Windows (1).Right) + 5;
    --  Console_Y := Int_C (Get_Windows (1).Top + 45);
    --  if Console_X + Console_Width > GetDeviceCaps (Context, SM_CYDLGFRAME) then
    --    Console_X := GetDeviceCaps (Context, SM_CYDLGFRAME) - Console_Width - 10;
    --  end if;
    --else
      Console_X := GetDeviceCaps (Context, SM_CYDLGFRAME) / 2 - Console_Width  / 2;
      Console_Y := GetDeviceCaps (Context, SM_CXHTHUMB)   / 2 - Console_Height / 2;
    --end if;
    ...
    Outter: loop
      ...
      if Lines /= Current_Lines then
        ...
        if Length (Current_Log) > Input_Buffer'Length then -- This needs cleanup !!!
          Cutoff       := Index (Current_Log, To_Str (ASCII.CR), Input_Buffer'Length) + 1;
          Cutoff_Lines := Count (Head  (Current_Log, Cutoff), To_Str (ASCII.CR));
          Current_Log  := Tail  (Current_Log, Length (Current_Log) - Cutoff);
        end if;
