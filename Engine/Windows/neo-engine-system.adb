...
separate (Neo.Engine) package body System is
  ...
  function OS_Info_Internal return OS_Info_State is
    ...
    begin
      ...
      begin
        ...
        return (App_Name => Delete (U (To_Str (Folder)), 1, Index (To_Str (Folder), "\", Backward)), -- First-level
                Path_Sep => '\',
                Path     => U (To_Str (Folder)),
                Username => U (To_Str (Username)),
  ...
  -- Window titles
  APP_NAME           : constant Str   := S (OS_Info.App_Name);
  ...
  -- Asset paths
  WIN32_PATH_ICON            : aliased Str_C := To_Str_C (OS_Info.Path & "\" & PATH_ASSETS & "\" & PATH_ICON             & ".ico");
  WIN32_PATH_CURSOR_ACTIVE   : aliased Str_C := To_Str_C (OS_Info.Path & "\" & PATH_ASSETS & "\" & PATH_CURSOR_ACTIVE    & ".cur");
  WIN32_PATH_CURSOR_INACTIVE : aliased Str_C := To_Str_C (OS_Info.Path & "\" & PATH_ASSETS & "\" & PATH_CURSOR_INACTIVE  & ".cur");
  ...
  --function Get_Vulkan_Extensions return Array_Str_Unbound is ((U (VK_KHR_SURFACE_EXTENSION_NAME), U (VK_KHR_WIN32_SURFACE_EXTENSION_NAME)));
  ...
  function Ok (Name, Message : Str; Buttons : Buttons_Kind; Icon : Icon_Kind) return Bool is
    ...
    function CBTProc (nCode : Int_C; wParam, lParam : Int_Ptr) return Int_Ptr is
      ...
      begin
        ...
        if nCode = HCBT_ACTIVATE and S (Class_Name) = S (DIALOG_CLASS) and S (Window_Text) = Name then
