      ...
      -----
        ...
        if DO_SET_BLANK_MOUSE then
          ...
          if Blank_Mouse = NULL_ADDRESS then
            Put_Debug("Warning, could not find the blank cursor at " & BLANK_MOUSE_PATH & ", ignoring.");
    ...
    function Handle_Events(
      ...
      begin
        if Index /= 0 and then Multi_Monitor_Windows = null or Index > Multi_Monitor_Windows'size then
          Put_Debug_Line(L("Handle_Events is being passed bad indexes!"));
          return False;
        end if;
        if
        Peek_Message(
    ...
    procedure Adjust(
      ...
      begin
        ...
        if Do_Fullscreen then
          ...
          Get_Version < Windows_2_6_System
          then
            if
            Change_Window_Setting(
              Window  => Primary_Window,
