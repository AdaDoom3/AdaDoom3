                                    ...
                                    JNIEnv* env, jclass cls)
{
    ...
    mAudioEnv = env;
    __android_log_print(ANDROID_LOG_INFO, "SDL", "Audio Thread: %i", getpid ());
...
static bool Android_JNI_ExceptionOccurred(JNIEnv *env)
{
...
static int Android_JNI_FileOpen(SDL_RWops* ctx)
{
    ...
    if (mJavaVM->GetEnv((void**) &env, JNI_VERSION_1_4) != JNI_OK) {
        LOGE("Failed to get the environment using GetEnv(); assuming main thread");
        env = mEnv;
    }
    ...
    assetManager = env->CallObjectMethod(context, mid);

    // inputStream = assetManager.open(<filename>);
    mid = env->GetMethodID(env->GetObjectClass(assetManager),
            "open", "(Ljava/lang/String;)Ljava/io/InputStream;");
    inputStream = env->CallObjectMethod(assetManager, mid, fileNameJString);
    if (Android_JNI_ExceptionOccurred(env)) {
        goto failure;
    }

    ctx->hidden.androidio.inputStream = inputStream;
    ctx->hidden.androidio.inputStreamRef = env->NewGlobalRef(inputStream);
    ...
    mid = env->GetMethodID(env->GetObjectClass(inputStream),
            "available", "()I");
    ctx->hidden.androidio.size = env->CallIntMethod(inputStream, mid);
    if (Android_JNI_ExceptionOccurred(env)) {
        goto failure;
    }

    // readableByteChannel = Channels.newChannel(inputStream);
    channels = env->FindClass("java/nio/channels/Channels");
    mid = env->GetStaticMethodID(channels,
            "newChannel",
    ...
    ctx->hidden.androidio.readableByteChannelRef =
        env->NewGlobalRef(readableByteChannel);

    // Store .read id for reading purposes
    mid = env->GetMethodID(env->GetObjectClass(readableByteChannel),
...
failure:
        ...
        env->DeleteGlobalRef((jobject)ctx->hidden.androidio.fileNameRef);

        if(ctx->hidden.androidio.inputStreamRef != NULL) {
            env->DeleteGlobalRef((jobject)ctx->hidden.androidio.inputStreamRef);
        }
    }
        ...
        const char* fileName, const char*)
{
    ...
    ctx->hidden.androidio.fileNameRef = env->NewGlobalRef(fileNameJString);
    ctx->hidden.androidio.inputStreamRef = NULL;
    env->DeleteLocalRef(fileNameJString);
        ...
        size_t size, size_t maxnum)
{
    ...
    jobject byteBuffer = env->NewDirectByteBuffer(buffer, bytesRemaining);

    while (bytesRemaining > 0) {
        // result = readableByteChannel.read(...);
        int result = env->CallIntMethod(readableByteChannel, readMethod, byteBuffer);
...
static int Android_JNI_FileClose(SDL_RWops* ctx, bool release)
{
    ...
    if (ctx) {
        ...
        // inputStream.close();
        jmethodID mid = env->GetMethodID(env->GetObjectClass(inputStream),
                "close", "()V");
        env->CallVoidMethod(inputStream, mid);
        env->DeleteGlobalRef((jobject)ctx->hidden.androidio.inputStreamRef);
        env->DeleteGlobalRef((jobject)ctx->hidden.androidio.readableByteChannelRef);
        if (Android_JNI_ExceptionOccurred(env)) {
            result = -1;
        }
...
extern "C" void *Android_JNI_GetActivityClass () {
    return mActivityClass;
}

extern "C" void *Android_JNI_GetEnv () {
    return mEnv;
}
